...
(function($) {
	$.extend($.fn.datagrid.defaults.editors, {
		...
		/**
		 * A dialog jumped out for editing lob data.
		 */
		lobDialog : {
			init : function(container, options) {
				var field = options.field;
				var input = $('<textarea class="datagrid-editable-input" readonly="true">').click(function() {
					var $window = $.fn.datagrid.defaults.editors.lobDialog.static_window;
					if ($window) {
						$window.remove();
					}
					$textarea = $('<textarea rows="25" cols="142">');
					$textarea.val(input.val());
					$window = $('<div>').append($textarea).appendTo($(this).parent()).window({
						width : 900,
						height : 395,
						minimizable : false,
						maximizable : false,
					...
					$.fn.datagrid.defaults.editors.lobDialog.static_window = $window;
				}).appendTo(container);
				return input;
			},
			destroy : function(target) {
